...
This skill ships with the `mdcc` plugin, which puts a pinned `mdcc` command on your PATH — just run `mdcc …` directly (no install needed, and the macOS library path is handled for you). The first run resolves and caches the pinned build, so it may take a few extra seconds.

If a compile fails with **missing system libraries** (e.g. WeasyPrint/pango/gobject errors during PDF generation, or `mmdc` not found for diagrams), run the `/mdcc-setup` slash command once to install the system dependencies.
...
Mermaid fenced code blocks are rendered to inline SVG images. Requires `mmdc` (mermaid-cli) installed and on PATH — run `/mdcc-setup` to install it (`npm install -g @mermaid-js/mermaid-cli`).
...
**macOS WeasyPrint note:** The plugin's `mdcc` launcher already sets `DYLD_FALLBACK_LIBRARY_PATH=/opt/homebrew/lib` for you, so you should not need to export it manually. If PDF generation still fails with *missing external libraries*, the C libraries themselves are not installed — run `/mdcc-setup`.
